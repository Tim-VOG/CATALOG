...
import { Check, WifiOff, AlertTriangle, ShoppingCart, Plus, CreditCard, X } from 'lucide-react'
...
// ── Options Dialog (shown before adding phone/router to cart) ──
...
export function ProductCard({ product, reservedQty = 0 }) {
  ...
  return (
    <>
        ...
        <Card spotlight={!isUnavailable} className={cn(
          ...
          <div className="relative aspect-[4/3] overflow-hidden bg-muted">
            <BlurImage
              ...
              alt={product.name}
              className={cn('transition-transform duration-300', !isUnavailable && 'group-hover:scale-105')}
            />
            ...
            {isUnavailable && (
              <div className="absolute inset-0 flex items-center justify-center bg-background/60">
                <span className="text-sm font-semibold text-destructive bg-background/80 px-3 py-1 rounded-full">Unavailable</span>
              </div>
            )}
            {!isUnavailable && (
              <div className="absolute bottom-2.5 right-2.5 opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                >
                  {inCart ? <><Check className="h-3 w-3" /> In Cart</> : <><Plus className="h-3 w-3" /> Add to Cart</>}
                </Button>
              </div>
            )}
          </div>
          ...
          <CardContent className="p-5 space-y-3 flex-1 flex flex-col">
            ...
            {product.wifi_only && (
              <div className="flex items-center gap-1 text-xs text-warning"><WifiOff className="h-3 w-3" /> WiFi only - No 4G/5G</div>
            )}
            {product.printer_info && (
              <div className="flex items-center gap-1 text-xs text-warning"><AlertTriangle className="h-3 w-3" /> B&W Laser - Print only</div>
            )}

            <div className="pt-2 border-t mt-auto flex items-center justify-between">
              <div className="text-sm">
                <span className={cn('font-bold', available > 3 ? 'text-success' : available > 0 ? 'text-warning' : 'text-destructive')}>
              ...
              {!isUnavailable && (
                <Button
                  ...
                  className={cn('h-7 w-7 sm:hidden shrink-0', inCart && 'bg-emerald-500 hover:bg-emerald-600 border-emerald-500')}
                  onClick={handleAddToCart}
                  disabled={addToCart.isPending}
                >
      ...
      {/* Options dialog for phone/router */}
      {optionsOpen && (
        <OptionsDialog
          product={product}
          open={optionsOpen}
