...
function OptionSection({ title, icon: Icon, options, optionType, onCreate, onUpdate, onDelete }) {
  ...
  const openCreate = () => { setEditing(null); setLabel(''); setShowDialog(true) }
  const openEdit = (opt) => { setEditing(opt); setLabel(opt.label); setShowDialog(true) }
  ...
  const handleSave = async () => {
    ...
    try {
      if (editing) {
        ...
        await onCreate({ option_type: optionType, label: label.trim(), value, sort_order: options.length })
  ...
  const handleToggle = async (opt) => {
    try { await onUpdate({ id: opt.id, is_active: !opt.is_active }) }
    catch (err) { showToast(err.message, 'error') }
  }

  const handleDelete = async (opt) => {
    if (!confirm(`Delete "${opt.label}"?`)) return
    try { await onDelete(opt.id); showToast('Option deleted') }
    catch (err) { showToast(err.message, 'error') }
  }
    ...
    <Card>
        ...
        {options.length === 0 ? (
          ...
          <div className="space-y-1.5">
            {options.map((opt) => (
              <div
                key={opt.id}
                className={`flex items-center gap-3 p-2.5 rounded-lg border text-sm ${!opt.is_active ? 'opacity-50' : ''}`}
              >
                ...
                <div className="flex items-center gap-0.5">
                  <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => handleToggle(opt)}>
                    <Badge className={opt.is_active ? 'bg-green-500/20 text-green-400 text-[9px]' : 'bg-red-500/20 text-red-400 text-[9px]'}>
      ...
      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle className="text-sm">{editing ? 'Edit Option' : `Add ${title.slice(0, -1)}`}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div className="space-y-1.5">
              <Label className="text-xs">Label *</Label>
              <Input value={label} onChange={(e) => setLabel(e.target.value)} placeholder="e.g. USB Hub" onKeyDown={(e) => e.key === 'Enter' && handleSave()} />
            </div>
          </div>
          <DialogFooter>
...
function SubscriptionSection({ plans, onCreate, onUpdate, onDelete }) {
  ...
  const openCreate = () => { setEditing(null); setForm({ name: '', type: 'call', price: '', description: '' }); setShowDialog(true) }
  const openEdit = (plan) => { setEditing(plan); setForm({ name: plan.name, type: plan.type, price: plan.price, description: plan.description || '' }); setShowDialog(true) }

  const handleSave = async () => {
    if (!form.name.trim() || !form.price.trim()) { showToast('Name and price are required', 'error'); return }
    try {
      if (editing) { await onUpdate({ id: editing.id, ...form }); showToast('Plan updated') }
      else { await onCreate(form); showToast('Plan created') }
      setShowDialog(false)
    } catch (err) { showToast(err.message, 'error') }
  }

  const handleToggle = async (plan) => {
    try { await onUpdate({ id: plan.id, is_active: !plan.is_active }) }
    catch (err) { showToast(err.message, 'error') }
  }

  const handleDelete = async (plan) => {
    if (!confirm(`Delete "${plan.name}"?`)) return
    try { await onDelete(plan.id); showToast('Plan deleted') }
    catch (err) { showToast(err.message, 'error') }
  }
    ...
    <Card>
      ...
      <CardContent className="pt-0">
        {plans.length === 0 ? (
          ...
          <div className="space-y-1.5">
            {plans.map((plan) => (
              <div key={plan.id} className={`flex items-center gap-3 p-2.5 rounded-lg border text-sm ${!plan.is_active ? 'opacity-50' : ''}`}>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="font-medium text-xs">{plan.name}</span>
                    <Badge className={`text-[9px] ${typeColor(plan.type)}`}>{typeLabel(plan.type)}</Badge>
                  </div>
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className="text-[11px] text-muted-foreground">{plan.price}</span>
                    {plan.description && <span className="text-[10px] text-muted-foreground truncate max-w-[200px]">— {plan.description}</span>}
                  </div>
                </div>
                <div className="flex items-center gap-0.5">
                  <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => handleToggle(plan)}>
                    <Badge className={plan.is_active ? 'bg-green-500/20 text-green-400 text-[9px]' : 'bg-red-500/20 text-red-400 text-[9px]'}>
      ...
      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle className="text-sm">{editing ? 'Edit Plan' : 'Add Subscription Plan'}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div className="space-y-1.5">
              <Label className="text-xs">Name *</Label>
              <Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="e.g. 5GB Data Plan" />
            </div>
            <div className="space-y-1.5">
            ...
            <div className="space-y-1.5">
              ...
              <Input value={form.price} onChange={(e) => setForm({ ...form, price: e.target.value })} placeholder="e.g. 29€/month" />
            </div>
            <div className="space-y-1.5">
...
export function OptionsTab() {
  ...
  if (optionsLoading || plansLoading) {
    return (
      <div className="space-y-4">
  ...
  return (
    <div className="space-y-4">
      <p className="text-xs text-muted-foreground">Manage accessories, software, and subscription plans available during checkout</p>
